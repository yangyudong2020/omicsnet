...
def check_metric(y_true, y_pred, name, ans_num):
    ...
    logging_info(f"for cls {name}, the Adjusted Rand Index (ARI): {ari}")
    logging_info(f"for cls {name}, the Adjusted Mutual Information (MI): {mi}")
    logging_info(f"for cls {name}, the Homogeneity: {homogeneity}")
    logging_info(f"for cls {name}, the Completeness: {completeness}")
    logging_info(f"for cls {name}, the V-measure: {v_measure}")
...
def cluster(NO_USE_NAME):
    ...
    kmeans = KMeans(n_clusters=NUM_SHAPE_CLUSTER, max_iter=100, init='k-means++')
    ans_cluster = kmeans.fit_predict(features)
