...
from batchgenerators.utilities.file_and_folder_operations import subfiles, join, save_json, load_json, \
    isfile
from nnunetv2.configuration import default_num_processes, dataset_num
...
from nnunetv2.evaluation.utils_radiomics_small import *
...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    img_origin_file = os.path.join(f'./nnUNet_raw/Dataset{dataset_num}/imagesTr',
    ...
    results['metrics'] = {}
    # print('shape is ',seg_ref.shape, seg_pred.shape, img_origin.shape, probs.shape)
    # print('unique is ',np.unique(seg_ref), np.unique(seg_pred), np.unique(img_origin), np.unique(probs))
    # print(f'labels_or_regions is {labels_or_regions}')
    seg_pred = calcu_seg_pred(seg_pred, img_origin, probs)
    # print('mask_ref is ', np.unique(seg_ref), )
    # print('mask_pred is ', np.unique(seg_pred))
    for r in labels_or_regions:
        ...
        num_mask_ref, num_mask_pred = mask_ref*1, mask_pred*1
        results['metrics'][r]['r2'] = r2_score(
            num_mask_ref.flatten(), num_mask_pred.flatten())
        results['metrics'][r]['rmse'] = np.sqrt(
            mean_squared_error(num_mask_ref.flatten(), num_mask_pred.flatten()))
        results['metrics'][r]['precision'] = tp / \
            (tp + fp) if tp + fp > 0 else np.nan
        results['metrics'][r]['recall'] = tp / \
            (tp + fn) if tp + fn > 0 else np.nan
        # print('mask_pred mask_ref shape is', mask_pred.shape, mask_ref.shape)
        results['metrics'][r]['hd95'] = metric.binary.hd95(mask_pred.squeeze(
        ), mask_ref.squeeze()) if mask_pred.sum() > 0 and mask_ref.sum() > 0 else np.nan
