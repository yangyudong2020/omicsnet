...
import numpy as np
import logging
from nnunetv2.configuration import USE_RADIOMICS, NUM_PROCESSOR_RADIO
...
def calcu_seg_pred(seg_pred, img_origin, probs, show_path=None):
    ...
    if USE_RADIOMICS:
        results, mask_list, is_left = calculate_class(
            ((seg_pred != 0)*1).astype('uint8'), img_origin)
        mask = np.zeros_like(seg_pred)
        for j, label in enumerate(results):
            ans = ((mask_list[j])*label).astype('int32')
            if is_left[j]:
...
def calculate_class(predictions, images):
    results = []
    mask_list = []
    is_left = []
    results_pool_work = []
    ...
    for i, lbl in enumerate(predictions):
        ...
        img1, img2 = images[i, :, 0:256], images[i, :, 256:512]
        ...
        for contour in contours:
            ...
            if len(num_t) == 1:
                continue
            results_pool_work[i].append(
                [contour_mask*img1, contour_mask.copy(), contour])
            mask_list[i].append(contour_mask)
            is_left[i].append(True)
        ...
        for contour in contours:
            ...
            mask_list[i].append(contour_mask)
            is_left[i].append(False)
        num_work += len(results_pool_work[i])
    print(f'num work is {num_work}')
    num_work = max(int(num_work), 1)
    results_pool = []
    global KMEANS_LIST, LDA_LIST, CLASS_ANS
    with mp.get_context('spawn').Pool(processes=min(NUM_PROCESSOR_RADIO, num_work)) as pool:
        worker_list = [i for i in pool._pool]
        for i, lbl in tqdm(enumerate(predictions), total=len(predictions)):
            results_pool.append([])
            for args in results_pool_work[i]:
                mask_img, contour_mask, contour = args
                proceed = not check_workers_alive_and_busy(
                    pool, worker_list, results_pool, allowed_num_queued=2)
                while not proceed:
                    sleep(0.1)
                    proceed = not check_workers_alive_and_busy(
                        pool, worker_list, results_pool, allowed_num_queued=2)
                ...
                gc.collect()
                results_pool[i].append(pool.starmap_async(
                    get_class, ((LDA_LIST, KMEANS_LIST, CLASS_ANS, mask_img, contour_mask, contour),)))

        for i, lbl in enumerate(predictions):
            if USE_POOL:
                results[i] = [r.get()[0] for r in results_pool[i]]
            else:
                results[i] = [r for r in results_pool[i]]
    return results, mask_list, is_left
