...
import numpy as np
import logging
from nnunetv2.configuration import USE_RADIOMICS, FULL_HEIGHT, FULL_WIDTH 

# from datasets.dataset_synapse import Synapse_dataset, RandomGenerator

radiomics.logger.setLevel(logging.NOTSET)
TRAIN_DIR = './weights/me_small_train'
...
def calculate_class(lbl, images):
    ...
    for args in tqdm(results_pool_work):
...
def merge(mask, lbl_prob, lbl):
    ...
    ans = np.zeros((FULL_HEIGHT, FULL_WIDTH)).astype('uint8')
    for i in range(FULL_HEIGHT):
        for j in range(FULL_WIDTH):
            if lbl[i, j] == mask[i, j]:
                ans[i, j] = mask[i, j]
            elif lbl[i, j] == 1:
                ans[i, j] = lbl[i, j] if lbl_prob[i, j] >= 0.3 else mask[i, j]
            elif lbl[i, j] == 2:
                ans[i, j] = lbl[i, j] if lbl_prob[i, j] >= 0.3 else mask[i, j]
            elif lbl[i, j] == 3:
                ans[i, j] = lbl[i, j] if lbl_prob[i, j] >= 0.3 else mask[i, j]
