...
from nnunetv2.configuration import ANISO_THRESHOLD, default_num_processes, trainloader_num_processes, num_iterations_per_epoch, num_epochs, num_valid, start_num_valid, USING_3D_FOR_UNETRPP, initial_learning_rate, USING_CLIP, COMPARING_MODEL_TYPE
...
if COMPARING_MODEL_TYPE == 'unetr':
    from nnunetv2.training.nnUNetTrainer.synapse.compare_model.unetr_2d import UNETR as trainerNetwork
elif COMPARING_MODEL_TYPE == 'missformer':
    from nnunetv2.training.nnUNetTrainer.synapse.compare_model.missformer import MISSFormer as trainerNetwork
elif COMPARING_MODEL_TYPE == 'nnformer':
    from nnunetv2.training.nnUNetTrainer.synapse.compare_model.nnformer import nnFormer as trainerNetwork
elif COMPARING_MODEL_TYPE == 'swinunet':
    from nnunetv2.training.nnUNetTrainer.synapse.compare_model.swinunet import SwinUnet as trainerNetwork
elif COMPARING_MODEL_TYPE == 'transunet':
    from nnunetv2.training.nnUNetTrainer.synapse.compare_model.transunet import VisionTransformer as trainerNetwork
elif COMPARING_MODEL_TYPE == 'unet':
    from nnunetv2.training.nnUNetTrainer.synapse.compare_model.unet import UNet as trainerNetwork
...
class nnUNetTrainer(object):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.network = trainerNetwork(in_channels=self.num_input_channels,
                             out_channels=len(label_manager._get_all_labels()),
                             img_size=self.configuration_manager.patch_size).cuda()
            ...
            if epoch % self.num_valid == 0 and epoch > start_num_valid:
                with torch.no_grad():
                    ...
                    if cur_metrics > self.best_dice_value:
                        ...
                        print(f'Yalp!!!, save best checkpoint and best dice is {self.best_dice_value}')
                        self.save_checkpoint(
                            join(self.output_folder, 'checkpoint_best.pth'))
