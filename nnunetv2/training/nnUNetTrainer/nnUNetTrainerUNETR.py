...
from nnunetv2.configuration import ANISO_THRESHOLD, default_num_processes, trainloader_num_processes, num_iterations_per_epoch, num_epochs, num_valid, start_num_valid, USING_3D_FOR_UNETRPP, initial_learning_rate, USING_CLIP, USING_ADJUST_LOSS
...
from nnunetv2.training.nnUNetTrainer.synapse.clip_utils import ClipLoss
...
if USING_3D_FOR_UNETRPP:
    from nnunetv2.training.nnUNetTrainer.synapse.unetr_pp_synapse_3d import UNETR_PP
else:
    from nnunetv2.training.nnUNetTrainer.synapse.unetr_pp_synapse import UNETR_PP


class AdjustLoss(nn.Module):
    def __init__(self, has_regions, get_deep_supervision_scales, ignore_label, manager_batch_dice, is_adjust) -> None:
        super().__init__()
        self.feature_loss = ClipLoss()
        if has_regions:
            loss = DC_and_BCE_loss({},
                                   {'batch_dice': manager_batch_dice,
                                    'do_bg': True, 'smooth': 1e-5, 'ddp': False},
                                   use_ignore_label=ignore_label is not None,
                                   dice_class=MemoryEfficientSoftDiceLoss)
        else:
            loss = DC_and_CE_loss({'batch_dice': manager_batch_dice,
        ...
        if is_adjust:
            self.alpha_1 = nn.Parameter(torch.ones(1), requires_grad=True).cuda()
            self.alpha_2 = nn.Parameter(torch.ones(1), requires_grad=True).cuda()
        else:
            self.alpha_1 = 0.5
            self.alpha_2 = 0.5

    def forward(self, img_feature, radio_feature, logit_scale, output, target):
        f_loss = self.feature_loss(
            img_feature, radio_feature, logit_scale)
        l = self.loss(output, target)*self.alpha_1 + f_loss*self.alpha_2
        return l
...
class nnUNetTrainer(object):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            label_manager = self.plans_manager.get_label_manager(
                self.dataset_json)
            self.network = UNETR_PP(in_channels=self.num_input_channels,
                                    out_channels=len(
                                        label_manager._get_all_labels()),
            ...
            if USING_CLIP:
                self.loss = AdjustLoss(self.label_manager.has_regions, self._get_deep_supervision_scales(
                ), self.label_manager.ignore_label, self.configuration_manager.batch_dice, USING_ADJUST_LOSS)
            else:
    ...
    def get_dataloaders(self):
        ...
        self.init_predictor_valid()
        if trainloader_num_processes == 0:
            mt_gen_train = SingleThreadedAugmenter(dl_tr, tr_transforms)
            mt_gen_val = SingleThreadedAugmenter(dl_val, val_transforms)
        else:
            mt_gen_train = LimitedLenWrapper(self.num_iterations_per_epoch, data_loader=dl_tr, transform=tr_transforms,
                                             num_processes=trainloader_num_processes, num_cached=12, seeds=None,
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        if USING_CLIP:
            feature = torch.tensor(feature).to(self.device, non_blocking=True)
        else:
            feature = None
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            if USING_3D_FOR_UNETRPP:
                ...
                indices_to_remove = {1, 4, 5}
            target = [item for idx, item in enumerate(
                target) if idx not in indices_to_remove]
            if USING_CLIP:
                # print('features is', torch.unique(feature))
                output, radio_feature, img_feature, logit_scale = self.network(
                    data, feature)
                l = self.loss(img_feature, radio_feature,
                              logit_scale, output, target)
            else:
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        # print(f'out unique is {torch.unique(output)}')
        # print(f'target shape is {target.shape}')
        # print(f'target unique is {torch.unique(target)}')
    ...
    def init_predictor_valid(self):
        self.predictor = nnUNetPredictor(tile_step_size=0.5, use_gaussian=True, use_mirroring=True,
                                         perform_everything_on_gpu=True, device=self.device, verbose=False,
                                         verbose_preprocessing=False, allow_tqdm=False)
        self.dataset_val = nnUNetDataset(self.preprocessed_dataset_folder, self.val_keys_pred,
                                         folder_with_segs_from_previous_stage=self.folder_with_segs_from_previous_stage,
                                         num_images_properties_loading_threshold=0)

    def perform_actual_validation(self, epoch_num, save_probabilities: bool = False):
        self.set_deep_supervision_enabled(False)
        self.network.eval()
        self.predictor.manual_initialization(self.network, self.plans_manager, self.configuration_manager, None,
                                             self.dataset_json, self.__class__.__name__,
                                             self.inference_allowed_mirroring_axes)

        with multiprocessing.get_context("spawn").Pool(min(default_num_processes, len(self.dataset_val))) as segmentation_export_pool:
            ...
            for k in self.dataset_val.keys():
                ...
                data, seg, properties, feature = self.dataset_val.load_case(k)
                ...
                try:
                    prediction = self.predictor.predict_sliding_window_return_logits(
                        data)
                except RuntimeError:
        ...
        if self.local_rank == 0:
            metrics = compute_metrics_on_folder(join(self.preprocessed_dataset_folder_base, 'gt_segmentations'),
                                                ...
                                                join(
                                                    validation_output_folder, f'summary_{epoch_num}.json'),
        ...
        return metrics['foreground_mean']["Dice"]
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            if epoch % self.num_valid == 0 and epoch > start_num_valid:
                with torch.no_grad():
                    ...
                    cur_metrics = self.perform_actual_validation(epoch, True)
                    if cur_metrics > self.best_dice_value:
                        self.best_dice_value = cur_metrics
                        print(
                            f'Yalp!!!, save best checkpoint and best dice is {self.best_dice_value}')
                        self.save_checkpoint(
                            join(self.output_folder, 'checkpoint_best.pth'))
