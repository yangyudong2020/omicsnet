...
class SegU_decoder(nn.Module):
    def __init__(self, input_size, in_out_chan, heads, reduction_ratios, n_class=9, norm_layer=nn.LayerNorm, is_last=False):
        ...
        if not is_last:
            ...
            self.layer_up = PatchExpand(
                input_resolution=input_size, dim=out_dim, dim_scale=2, norm_layer=norm_layer)
        ...
        self.layer_former_1 = TransformerBlock(
            out_dim, heads, reduction_ratios)
    ...
    def forward(self, x1, x2=None):
        if x2 is not None:
            ...
            if self.last_layer:
                out = self.last_layer(self.layer_up(tran_layer_2).view(
...
class BridegeBlock_4(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        sk2 = bridge4[:, 3136:4704, :].reshape(
            B, 28, 28, C*2).permute(0, 3, 1, 2)
        sk3 = bridge4[:, 4704:5684, :].reshape(
            B, 14, 14, C*5).permute(0, 3, 1, 2)
        sk4 = bridge4[:, 5684:6076, :].reshape(
            B, 7, 7, C*8).permute(0, 3, 1, 2)
...
class BridegeBlock_3(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        sk3 = bridge4[:, 1568:2548, :].reshape(
            B, 14, 14, C*5).permute(0, 3, 1, 2)
        sk4 = bridge4[:, 2548:2940, :].reshape(
            B, 7, 7, C*8).permute(0, 3, 1, 2)
...
class MISSFormer(nn.Module):
    def __init__(self, in_channels, out_channels, img_size, token_mlp_mode="mix_skip", encoder_pretrained=False):
        super().__init__()
        ...
        self.decoder_3 = MyDecoderLayer((d_base_feat_size, d_base_feat_size),
                                        in_out_chan[3], heads[3], reduction_ratios[3], token_mlp_mode, n_class=num_classes)
        self.decoder_2 = MyDecoderLayer((d_base_feat_size*2, d_base_feat_size*2),
                                        in_out_chan[2], heads[2], reduction_ratios[2], token_mlp_mode, n_class=num_classes)
        self.decoder_1 = MyDecoderLayer((d_base_feat_size*4, d_base_feat_size*4),
                                        in_out_chan[1], heads[1], reduction_ratios[1], token_mlp_mode, n_class=num_classes)
        self.decoder_0 = MyDecoderLayer((d_base_feat_size*8, d_base_feat_size*8),
                                        in_out_chan[0], heads[0], reduction_ratios[0], token_mlp_mode, n_class=num_classes, is_last=True)
    ...
    def forward(self, x):
        ...
        # print('out shape is', tmp_0.shape)
        if self.do_ds:
            return [tmp_0]
        else:
            return tmp_0
...
class EfficientSelfAtten(nn.Module):
    ...
    def forward(self, x: torch.Tensor, H, W) -> torch.Tensor:
        B, N, C = x.shape
        q = self.q(x).reshape(B, N, self.head, C //
                              self.head).permute(0, 2, 1, 3)
        ...
        kv = self.kv(x).reshape(B, -1, 2, self.head, C //
                                self.head).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
...
class Scale_reduce(nn.Module):
    def __init__(self, dim, reduction_ratio):
        ...
        if (len(self.reduction_ratio) == 4):
            self.sr0 = nn.Conv2d(
                dim, dim, reduction_ratio[3], reduction_ratio[3])
            self.sr1 = nn.Conv2d(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if (len(self.reduction_ratio) == 4):
            ...
            tem1 = x[:, 3136:4704, :].reshape(
                B, 28, 28, C*2).permute(0, 3, 1, 2)
            tem2 = x[:, 4704:5684, :].reshape(
                B, 14, 14, C*5).permute(0, 3, 1, 2)
            tem3 = x[:, 5684:6076, :]
...
class LocalEnhance_EfficientSelfAtten(nn.Module):
    ...
    def forward(self, x: torch.Tensor, H, W) -> torch.Tensor:
        ...
        local_pos = self.local_pos(local_v).reshape(
            B, -1, self.head, C//self.head).permute(0, 2, 1, 3)
        x_atten = ((attn_score @ v) + local_pos).transpose(1,
                                                           2).reshape(B, N, C)
        out = self.proj(x_atten)
...
class MixD_FFN(nn.Module):
    def __init__(self, c1, c2, fuse_mode="add"):
        ...
        self.act = nn.GELU()
        self.fc2 = nn.Linear(
            c2, c1) if fuse_mode == "add" else nn.Linear(c2*2, c1)
        self.fuse_mode = fuse_mode

    def forward(self, x):
        ax = self.dwconv(self.fc1(x), H, W)
        fuse = self.act(
            ax+self.fc1(x)) if self.fuse_mode == "add" else self.act(torch.cat([ax, self.fc1(x)], 2))
        out = self.fc2(ax)
        return out
    ...
    def __init__(self, image_size, dims, layers, fuse_mode='add'):
        ...
        self.block1 = nn.ModuleList([
            FuseTransformerBlock(
                dims[0], heads[0], reduction_ratios[0], fuse_mode)
            for _ in range(layers[0])])
        self.norm1 = nn.LayerNorm(dims[0])

        self.block2 = nn.ModuleList([
            FuseTransformerBlock(
                dims[1], heads[1], reduction_ratios[1], fuse_mode)
            for _ in range(layers[1])])
        self.norm2 = nn.LayerNorm(dims[1])

        self.block3 = nn.ModuleList([
            FuseTransformerBlock(
                dims[2], heads[2], reduction_ratios[2], fuse_mode)
            for _ in range(layers[2])])
        self.norm3 = nn.LayerNorm(dims[2])

        self.block4 = nn.ModuleList([
            FuseTransformerBlock(
                dims[3], heads[3], reduction_ratios[3], fuse_mode)
            for _ in range(layers[3])])
        self.norm4 = nn.LayerNorm(dims[3])
...
class SegFormer(nn.Module):
    def __init__(self, model_name: str = 'B0', num_classes: int = 19, image_size: int = 224) -> None:
        super().__init__()
        assert model_name in segformer_settings.keys(
