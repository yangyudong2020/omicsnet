...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        # print('input x shape', x.shape, self.fc1)
        x = self.fc1(x)
        x = self.act(x)
        x = self.drop(x)
        x = self.fc2(x)
...
class SwinTransformerBlock_kv(nn.Module):
    ...
    def forward(self, x, mask_matrix, skip=None, x_up=None):
        ...
        print('for SwinTransformerBlock_kv input shape and self.input_resolution',
              x.shape, self.input_resolution)
        ...
        if self.shift_size > 0:
            skip = torch.roll(skip, shifts=(-self.shift_size, -
                              self.shift_size), dims=(1, 2))
            x_up = torch.roll(x_up, shifts=(-self.shift_size, -
                              self.shift_size), dims=(1, 2))
        ...
        skip = skip.view(-1, self.window_size * self.window_size,
                         C)
        x_up = window_partition(x_up, self.window_size)
        x_up = x_up.view(-1, self.window_size * self.window_size,
                         C)
...
class WindowAttention_kv(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        coords = torch.stack(torch.meshgrid([coords_s, coords_h]))
    ...
    def forward(self, skip, x_up, pos_embed=None, mask=None):
        ...
        kv = kv.reshape(B_, N, 2, self.num_heads, C //
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None, pos_embed=None):
        ...
        q = q * self.scale
        ak = k.transpose(-2, -1).contiguous()

        attn = (q @ ak)
        print('error in this ', self.relative_position_index.shape,
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        B, L, C = x.shape
        H, W = self.input_resolution
        print('for SwinTransformerBlock input shape and self.input_resolution',
              x.shape, self.input_resolution)
        assert L == H * W, "input feature has wrong size"
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        B, L, C = x.shape
        print('for PatchMerging input shape and self.input_resolution',
              x.shape, H, W)
        assert L == H * W, "input feature has wrong size"
        x = x.view(B, H, W, C)
...
class Patch_Expanding(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        B, L, C = x.shape
        print('for Patch_Expanding input shape and self.input_resolution',
              x.shape, H, W)
        assert L == H * W, "input feature has wrong size"
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.depth = depth
        # build blocks
        if input_resolution[0] in [3, 1]:
            x = input_resolution[0]
            input_resolution = (x+1, x+1)
...
class BasicLayer_up(nn.Module):
    ...
    def forward(self, x, skip, H, W):
        ...
        mask_windows = window_partition(img_mask, self.window_size)
        mask_windows = mask_windows.view(-1,
                                         self.window_size * self.window_size)  # 3d��3��winds�˻�����Ŀ�Ǻܴ�ģ�����winds����̫��
        attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
        attn_mask = attn_mask.masked_fill(
            attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))
...
class project(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.last:
            ...
            x = x.flatten(2).transpose(1, 2).contiguous()
            x = self.norm2(x)
            x = x.transpose(1, 2).contiguous().view(-1,
                                                    self.out_dim, Wh, Ww)
        return x
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=4, embed_dim=96, norm_layer=None):
        ...
        stride1 = [patch_size[0], patch_size[1]]
        stride2 = [patch_size[0], patch_size[1]]
        self.proj1 = project(in_chans, embed_dim//2, stride1,
                             1, nn.GELU, nn.LayerNorm, False)
        self.proj2 = project(embed_dim//2, embed_dim,
                             stride2, 1, nn.GELU, nn.LayerNorm, True)
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                input_resolution=(
                    (pretrain_img_size[0] // patch_size[0] // 2 ** (i_layer+2)), (pretrain_img_size[1] // patch_size[1] // 2 ** (i_layer+2))),
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers)[::-1]:
            layer = BasicLayer_up(
                ...
                input_resolution=(
                    (pretrain_img_size[0] // patch_size[0] // 2 ** (len(
                        depths)-i_layer-1)), (pretrain_img_size[1] // patch_size[1] // 2 ** (len(depths)-i_layer-1))),
                depth=depths[i_layer],
                num_heads=num_heads[i_layer],
                window_size=window_size[i_layer],
                mlp_ratio=mlp_ratio,
            ...
            self.layers.append(layer)
        self.num_features = [int(embed_dim * 2 ** i)
                             for i in range(self.num_layers)]
...
class nnFormer(nn.Module):
    ...
    def __init__(self,
        ...
        self.decoder = Decoder(pretrain_img_size=crop_size, embed_dim=embed_dim,
                               window_size=window_size[::-1][1:], patch_size=patch_size, num_heads=num_heads[::-1][1:], depths=depths[::-1][1:])

        self.final = []
        if self.do_ds:

            for i in range(len(depths)-1):
                self.final.append(final_patch_expanding(
