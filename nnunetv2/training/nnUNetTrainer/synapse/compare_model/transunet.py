...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        if config.patches.get("grid") is not None:   # ResNet
            ...
            print('img size and patch size', img_size, grid_size)
    ...
    def forward(self, hidden_states, features=None):
        ...
        for i, decoder_block in enumerate(self.blocks):
            if features is not None:
                ...
                skip = None
            # if skip is not None:
                # print('x shape is skip shape is', x.shape, skip.shape)
            x = decoder_block(x, skip=skip)
        return x


class VisionTransformer(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, img_size: 224, config=CONFIGS['R50-ViT-B_16'], num_classes=21843, zero_head=False, vis=False):
