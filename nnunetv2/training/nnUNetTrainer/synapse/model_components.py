...
from nnunetv2.training.nnUNetTrainer.synapse.transformerblock import TransformerBlock
...
class UnetrPPEncoder(nn.Module):
    def __init__(self, mul_factor=16, input_size=[32 * 32, 16 * 16, 8 * 8, 4 * 4], dims=[32, 64, 128, 256],
                 proj_size=[64, 64, 64, 32], depths=[3, 3, 3, 3],  num_heads=4, spatial_dims=2, in_channels=1, dropout=0.0, transformer_dropout_rate=0.15, **kwargs):
        super().__init__()

        for i in range(len(input_size)):
            input_size[i] *= mul_factor
        ...
        self.hidden_states = []
        self.feature_fc = [nn.Linear(61440, 4096), nn.LeakyReLU(), nn.Linear(4096, 512)]
